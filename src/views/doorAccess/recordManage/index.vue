<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="工号" prop="personCode" width="150" />
      <el-table-column label="姓名" prop="personName" />
      <el-table-column label="允许通行" prop="description" />
    ...
    <!-- 导出记录 -->
    <form id="exportForm" ref="exportForm" name="exportForm" method="post" :action="exportUrl" />
...
import { queryRecord, openRecordExport } from '@/api/record'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      queryRecord(this.query).then(response => {
        const { data } = response
        this.list = data.rows
    ...
    exportRecord () {
      ...
      this.exportUrl = openRecordExport()
      this.$nextTick(() => {
